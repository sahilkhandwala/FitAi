...
This project uses:
- **SQLite (WAL mode)** at `SQLITE_DB_PATH` — 10 tables, single file, no Cloud SQL
- **Prefect v3+** for pipeline orchestration — 6 scheduled flows, self-hosted on VM
- **No Spark, Kafka, or Flink** — single-user bot, data volumes are tiny
- **knowledge_base/** — JSON files loaded in full on every agent call (~6k tokens)
- **diskcache** — persistent local cache for callout dedup and Fitbit token
- **cachetools TTLCache** — in-memory for media buffer (2s) and health profile (1hr)

Relevant data flows for this project:
- Fitbit API → `daily_health_logs` table (morning_report_flow, 10:30am)
- Telegram photo → MealAnalyzerAgent → `meal_logs` table
- Lab report PDF → HealthExtractorAgent → `user_health_profile` + `user_nutrition_guidance`
- 90-day `meal_logs` + `daily_health_logs` → semantic_extraction_flow → `user_semantic_memory`
- `meal_logs` aggregation → DailySummaryAgent → `daily_summaries`

All DB access goes through `db/queries.py` — never raw SQL in agents or flows.
All 10 table schemas are in `docs/nutrition-tracker-design.md` under SQLite Schema.
